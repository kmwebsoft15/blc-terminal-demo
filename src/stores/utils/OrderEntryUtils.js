...
import { OrderTicket } from '../../lib/bct-ws';
...
export const SubmitOrderHandler = async (orderType) => {
    const { SubmitOrderTicket } = await OrderTicket({ throttleMs: 45, orderType });
...
export const OrderTicketPayload = (
    baseSymbol,
    quoteSymbol,
    amount,
    price,
...
const only1Dot = (s = '') => s.replace(/[.]/, '{dot}').replace(/[.]/, '').replace('{dot}', '.');
const cleanNumToStr = (s = '') => Number(s).toString().trim();

export const valueNormalized = (prevInput, input = '') => {
    let inputStr = String(input);
    let prevInputStr = String(prevInput);
...
export const postSubmissionSnackbarHandler = (
    ...
    {
        [ORDER_TICKET_SIDE]: side,
        [ORDER_TICKET_SIZE]: amount,
        [ORDER_TICKET_PRICE]: price,
        [ORDER_TICKET_SYMBOL]: symbol,
    }
) => {
    let baseSymbol = '';
    ...
    snackbar({
        message: () => (
            <React.Fragment>
                <span><b>{type} Order Submitted!</b></span> <br /><br />
                <span><b>Side:</b> {side} {baseSymbol}</span> <br />
                <span><b>Amount:</b> {customDigitFormat(amount)} {baseSymbol}</span> <br />
                <span><b>Price:</b> {customDigitFormat(price)} USDT</span> <br />
                <span><b>Total Price:</b> {customDigitFormat(amount * price)} USDT</span>
            </React.Fragment>
        ),
    });
};
