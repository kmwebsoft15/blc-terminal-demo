...
import {
    DropdownWrapper, SelectedItemLabel, Dropdown,
    ItemList, ListStyleWrapper, ListItem3, DropMenuIcon
} from './Components';
...
class ForexDropdown extends React.Component {
    ...
    itemCellRenderer = ({ rowData }) => {
        const symbol = (rowData.symbol || '').replace('F:', '');
        const isSelected = symbol === this.props.value;

        return (
            <ListItem3
                className={isSelected ? 'active' : ''}
                onClick={() => { this.handleSelectItem(symbol); }}
            >
                <span>{symbol}</span>
                <span>20%</span>
            </ListItem3>
